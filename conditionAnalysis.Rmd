...
```{r newData}
newData = FALSE
date <- 'May27' #suffix for data subdirectory
```
...
# build up hierarchical random effects for each species 
  reBySpecies <- function(mod){
    ...
    ggplot(reAll, aes(condval12, huc12)) +
      geom_point(color = 'orange') +
      geom_point(aes(condval8, huc12), color = 'blue') +
      geom_point(aes(condvalLoc, huc12, shape = factor(loc)), color = 'black') +
      geom_vline(xintercept = 0) +
...
##  
  ...
  datBySpecies <- dat %>%
    ...
    mutate(charts = map(model, reBySpecies))




## SDs of nested levels ################
  datBySpeciesSD <- datBySpecies %>%
    mutate(tidy = map(model, broom::tidy)) %>% 
    unnest(tidy) %>%
    select(species, estimate, term, group) %>%
    filter(!term == '(Intercept)')
  
  ggSD <- ggplot(datBySpeciesSD, aes(species,estimate, fill = group)) +
